require([
  ...
  $(window).scroll(function () {
    ...
    _.each(
      tablenames, // iterate over alle tables
      function (tablename) {
...
*/
          if (
            posTop > posTable &&
            posTop < posTable + table.height() - heightLastRow
          ) {
            // begin and end of the table has to be between the window
  ...
  var CustomVisibleRenderer = TableView.BaseCellRenderer.extend({
    canRender: function (cell) {
      // define which cells you want to operate on
  ...
  // All-Option in Multiselect verbessern
  function handleAllOptionForMultipleInputs(inputs, defaultOptionValue) {
    _.each(inputs, function (inputWithAllOpt) {
      ...
      input.on("change", function (newValue) {
        console.log(
          this.id + ": " + newValue + ", isArr: " + Array.isArray(newValue)
        );
        ...
        if (newValue.length === 0) {
          ...
          newValue.length > 1 &&
          newValue.indexOf(defaultOptionValue) == 0
        ) {
          newValue = newValue.filter(function (value) {
            return value != defaultOptionValue;
          });

          //if the element 'All' is selected
        } else if (
          newValue.length > 1 &&
          newValue.includes(defaultOptionValue)
        ) {
